...
- 会话开始:查询词库中无学习记录的单词,随机打乱,构建 `Deque<QueueItem>`;`QueueItem = (wordId, streak)`
- 点"认识":streak+1;streak==2 → 写 study_records、出队;否则以相同位置权重放回队尾方向(见下)
- 点"不认识":streak=0,插入到距队首第 3 位(剩余不足 3 张则队尾)
- 每次判定后检查:已背数达到当日上限 → 终止会话并提示;队列空 → 提示完成
- **替代方案**: 队列状态持久化到数据库(增加会话表,复杂度不成比例,否决);连续认识次数持久化(中途退出不丢中间进度,但课程演示无此必要,否决)
